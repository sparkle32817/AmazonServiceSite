...
class Service_magnet_keyword_search_model extends CI_Model
{
    ...
    public function startSearchKeyword($client_id, $service_name, $postData)
    {
        ...
        $this->db->insert('tbl_service_magnet_search_data', $arr);
    ...
    {
        $this->db->insert('tbl_service_magnet_search_complete', $arr);
    ...
    public function getDataInfo($id)
    {
        ...
        $str_query = 'SELECT *
						FROM `tbl_service_magnet_search_complete`
						WHERE (task_id, phrase) IN (
							SELECT id, keyword
							FROM `tbl_service_magnet_search_data`
    ...
    public function getMagnetDataInfo($id)
    {
        $str_query = 'SELECT a.keyword, b.name AS market
                        FROM tbl_service_magnet_search_data a
    ...
    public function getKeywordCount($id)
    {
        $task_id = $this->getRelatedTaskID($id);

        $str_query = 'SELECT COALESCE(COUNT(a.id), 0) AS cnt_phrase, COALESCE(b.cnt_organic, 0) AS cnt_organic, COALESCE(c.cnt_smart, 0) AS cnt_smart, COALESCE(d.cnt_amz, 0) AS cnt_amz
                        FROM tbl_service_magnet_search_complete a
                        LEFT JOIN(
                            SELECT task_id, COUNT(id) AS cnt_organic
                            FROM tbl_service_magnet_search_complete
                            WHERE organic=1
                            GROUP BY task_id
                        ) b ON a.task_id=b.task_id
                        LEFT JOIN(
                            SELECT task_id, COUNT(id) AS cnt_smart
                            FROM tbl_service_magnet_search_complete
                            WHERE smart_complete=1
                            GROUP BY task_id
                        ) c ON a.task_id=c.task_id
                        LEFT JOIN(
                            SELECT task_id, COUNT(id) AS cnt_amz
                            FROM tbl_service_magnet_search_complete
...
//                $this->db->not_like('phrase', $arr[$i]);
        ...
        if (!empty($postData['match_type']))
        {
            ...
            if (strpos($postData['match_type'],'Smart Complete Keyword')!== false)
                $this->db->where('smart_complete', 1);

            if (strpos($postData['match_type'],'Amazon Recommended Keywords')!== false)
                $this->db->where('amz_recommended', 1);
        }

//	    $this->db->get('tbl_service_magnet_search_complete');
//        return $this->db->last_query();
        return $this->db->get('tbl_service_magnet_search_complete')->result_array();
    }

    public function getHistory($client_name)
    {
        $service = $this->getServiceInfo('Magnet Related Keyword Search');
        $service_id = $service['id'];

        $str_query = 'SELECT a.id, a.market_place, b.keyword, a.status, a.request_time
						FROM tbl_view_all_task a
						JOIN tbl_service_magnet_search_data b ON b.id = a.`related_task_id` 
    ...
    public function deleteHistory($task_id)
    {
        ...
        $this->db->delete('tbl_service_magnet_search_data', array('id'=>$related_id));
        $this->db->delete('tbl_service_magnet_search_complete', array('asin_id'=>$related_id));
    ...
    {
        $result = $this->db->get_where('tbl_service_magnet_search_complete', array('task_id'=>$task_id, 'phrase'=>$keyword));
    ...
    public function getMarketAndFlag($id)
    {
        $str_query = 'SELECT b.name as market_place, b.country_code as flag
						FROM `tbl_task` a
						JOIN `tbl_market` b ON b.id=a.market_id
						WHERE a.id='.$id;
